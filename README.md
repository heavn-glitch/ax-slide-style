...
AX Skillaton의 디자인 토큰과 논리 표현 규칙으로 HTML 또는 PowerPoint 발표자료를 만드는 Codex Skill입니다.
...
- 사용자 선택에 따라 단일 HTML 또는 편집 가능한 PPTX 출력
...
```text
$ax-slide-style을 사용해 이 기획안을 편집 가능한 PPTX 발표자료로 만들어줘.
```

## 산출물

- HTML: CSS가 인라인된 단일 `deck.html`
- PPTX: 편집 가능한 `deck.pptx`

형식이 명시되지 않으면 스킬이 HTML과 PPTX 중 필요한 형식을 먼저 확인합니다. HTML의 Pretendard와 lucide는 CDN을 사용하므로 완전한 오프라인 실행이 필요하면 로컬 폰트와 인라인 SVG로 교체해야 합니다.
